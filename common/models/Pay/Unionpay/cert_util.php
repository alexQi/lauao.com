...
class CertUtil{
    ...
    private static function initSignCert($certPath, $certPwd){
        ...
        $realCertPath = __DIR__.'/'.$certPath;
        $pkcs12certdata = file_get_contents($realCertPath);

        if($pkcs12certdata === false ){
        	$logger->LogInfo($realCertPath . "读取失败。");
        }
...
//                     $cert->certId = $certId;
    ...
    public static function getVerifyCertByCertId($certId,$cartPath){
        if(count(CertUtil::$verifyCerts) == 0){
            self::initVerifyCerts($cartPath);
        }
        if(count(CertUtil::$verifyCerts) == 0){
            return null;
        }
