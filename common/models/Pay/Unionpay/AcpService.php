<?php
namespace app\models\Pay\Unionpay;
...
class AcpService {
	 ...
	 * @return 是否成功
	 */
	static function validate($params,$cartPath) {
		$public_key = CertUtil::getVerifyCertByCertId($params['certId'],$cartPath);
		...
		$res['params_str'] = $params_str;
        $res['signature'] = $signature;
        $res['params_sha1x16'] = $params_sha1x16;
	 ...
	 * @return 是否成功
	 */
	static function validateAppResponse($jsonData,$cartPath) {

		$data = json_decode($jsonData);
		$sign = $data->sign;
		$data = $data->data;
		$dataMap = parseQString($data);
		$public_key = CertUtil::getVerifyCertByCertId($dataMap['cert_id'],$cartPath);
